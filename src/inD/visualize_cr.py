...
from commonroad.visualization.plot_helper import set_non_blocking, redraw_obstacles, draw_object
...
if __name__ == '__main__':
    ...
    parser.add_argument("input", type=Path, help="Path to input file (.xml commonroad scenario)")
    parser.add_argument("-n", default=0, type=int, help="Number of time steps to show (default: 0 - show all frames)", dest="number")
    parser.add_argument("-s", default=3, type=int, help="Number of frames to skip each time step", dest="speed")
    parser.add_argument("-b", default=0, type=int, help="First frame to be drawn", dest="begin")
    parser.add_argument("-o", default="", type=str, help="Output directory for saved animation", dest="output")
    parser.add_argument("-l", action="store_true", help="Draw lanelet labels", dest="labels")
    parser.add_argument("-t", "--suffix", default="png", help="Filetype, suffix of the files to be written", dest="suffix")
    parser.add_argument("--successors_of", default="0", type=int, help="Only draw all successors of the lanelet with specified id")
    ...
    if args.number != 0:
        ...
        number_frames = max(1, 1, *[o.prediction.occupancy_set[-1].time_step for o in scenario.dynamic_obstacles], *[state.time_step.end for pp in planning_problem_set.planning_problem_dict.values() for state in pp.goal.state_list])
    ...
    for i in range(args.begin + 1, args.begin + number_frames):
        ...
        if WRITE:
            plt.savefig(output_d.joinpath("{:0{numdigits}d}.{}".format(i, suffix, numdigits=numdigits)))
