...
from data_converters.src.highD.highd_to_cr import create_highd_scenarios
from data_converters.src.inD.ind_to_cr import create_ind_scenarios
from data_converters.src.INTERACTION.interaction_to_cr import (
...
def get_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Generates CommonRoad scenarios different datasets",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument(
        "dataset",
        type=str,
        choices=["inD", "highD", "INTERACTION"],
        help="Specification of dataset",
    )
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Indicator if vehicles used for planning problem should be kept in scenario, " "default=False",
    )
    parser.add_argument(
        "--obstacle_start_at_zero",
        default=False,
        action="store_true",
        help="Indicator if the initial state of an obstacle has to start at time step zero, " "default=False",
    )
    parser.add_argument(
        "--num_processes",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Decrease dt by n*dt, works only for highD converter",
    )
    parser.add_argument(
        "--num_vertices",
        type=int,
...
def main(args):
    ...
    if args.dataset == "highD":
        create_highd_scenarios(
            args.input_dir,
            args.output_dir,
            args.num_time_steps_scenario,
            args.num_planning_problems,
            args.keep_ego,
            args.obstacle_start_at_zero,
            args.num_processes,
            args.downsample,
            args.num_vertices,
        )
